...
count
```
All samples are in the same order in both the datasets.
# Multi-omics data integration
For each integration method, follows a representation of the scaled distance matrix obtained from the similarity matrix given by integration (look at next step). The 2D plots are obtained by using Multidimensional Scaling on the distance matrices.
