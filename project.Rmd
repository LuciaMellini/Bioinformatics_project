...
## Integrated data through SNF
```{r}
#"medoids"    "id.med"     "clustering" "objective"  "isolation"  "clusinfo"   "silinfo"    "diss"       "call"       "data"   
sc.res <- SNFtool::spectralClustering(W_int_SNF, K=k)
#to have uniform type
sc.res <- pam(sc.res,k=k)
for(i in length(rownames(complete[1]))){
  print(rownames(complete[[1]])[i], sc.res$clustering[i])
  sc.res$clustering[i] <- c(rownames(complete[[1]])[i], sc.res$clustering[i])
}
