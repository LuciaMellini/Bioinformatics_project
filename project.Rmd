...
for (i in 1:length(complete)){
  ...
  for(j in unique(sc.res[[i]])){
    label[[j]] <- samples[sc.res[[i]][samples]==j]
    points_label[[j]] <- mds[rownames(mds) %in% label[[j]], ]
...
for(i in unique(sc.SNF.res)){
  label[[i]] <- samples[sc.SNF.res[samples]==i]
  points_label[[i]] <- mds[rownames(mds) %in% label[[i]], ]
