...
mo <- mo[,primary,]
```
We do not want to avoid working with replicates for the same sample, so we check that the barcode, the identifier for the sample, are unique in the multi-omics dataset. It emerges that indeed there are no replicated samples.
```{r}
...
mo <- mo[, no_ffpe, ] 
```
Obviously we want to look at the samples from three points of view, so we keep only those that are present in all three considered data views in the multi-omics dataset.
```{r}
...
```
It would also be ideal to not have to deal with missing values, so we get rid of those columns. It turns out that there are *NA*s only in the protein expression data view.
```{r}
