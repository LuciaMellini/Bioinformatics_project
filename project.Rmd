...
```{r}
for (i in 1:length(complete))
  clusplot(pam.res[[i]], main ="", sub=NULL, cex=0.9, col.p=pam.res[[i]]$cluster, plotchar=FALSE)
```
```{r}
for (i in 1:length(complete)){
...
```{r}
#tikz("myPlot.tex")
clusplot(pam.SNF.res, main ="", sub=NULL, cex=0.9, col.p=pam.SNF.res$cluster, plotchar=FALSE)
#dev.off()
```
```{r}
...
### Data integrated though mean
```{r}
clusplot(pam.mean.res, main="", sub=NULL, cex=0.9, col.p=pam.mean.res$cluster, plotchar=FALSE)
```
```{r}
